...
# Choose your LLM provider: "openai", "anthropic", "gemini", "deepseek", "groq", "ollama", "openrouter", "local"
LLM_PROVIDER = "local"
...
class LocalProvider(LLMProvider):
    def __init__(self):
        self._name = "Local heuristic judge"
    ...
    def complete(self, prompt: str, system: str = None) -> str:
        ...
        body = extract(r'Body \([^\)]*\): "([\s\S]*?)"', "")
        category = extract(r'Category: ([^\n]+)', "unknown")
        merchant = extract(r'Merchant: ([^\n]+)', "unknown")
        trigger_kind = extract(r'Trigger Kind: ([^\n]+)', "unknown")
        trigger_payload = extract(r'Trigger Payload: (\{[\s\S]*?\})', "{}")
        cta = extract(r'CTA: ([^\n]+)', "none")

        body_lower = body.lower()
        nums = len(re.findall(r'\d+', body))
        has_source = any(token in body_lower for token in ["source", "jida", "ida", "dci", "circular", "source:"])
        has_offer = any(token in body_lower for token in ["@ ₹", "₹", "free", "slots", "offer", "plan", "trial"])
        has_personalization = any(token in body_lower for token in [merchant.lower().split(" ")[0], "your ", "hi ", "dr.", "doc"])
        trigger_words = trigger_kind.lower().replace("_", " ")
        trigger_seen = trigger_words in body_lower or any(word in body_lower for word in trigger_words.split())

        category_fit = 6
        if category == "dentists" and any(tok in body_lower for tok in ["dr.", "jida", "fluoride", "caries", "recall"]):
            category_fit = 9
        elif category == "salons" and any(tok in body_lower for tok in ["bridal", "balayage", "hair spa", "keratin", "hi "]):
            category_fit = 9
        elif category == "restaurants" and any(tok in body_lower for tok in ["covers", "thali", "biryani", "delivery", "match-night"]):
            category_fit = 9
        elif category == "gyms" and any(tok in body_lower for tok in ["coach", "pt", "membership", "hiit", "churn"]):
            category_fit = 9
        elif category == "pharmacies" and any(tok in body_lower for tok in ["molecule", "batch", "generic", "refill", "pharmacist"]):
            category_fit = 9

        specificity = min(10, 3 + nums * 2 + (2 if has_source else 0) + (1 if has_offer else 0))
        merchant_fit = 5 + (3 if has_personalization else 0)
        decision_quality = 5 + (3 if trigger_seen else 0) + (1 if cta.lower() in {"yes", "open_ended"} else 0)
        engagement = 5 + (2 if any(tok in body_lower for tok in ["want me", "reply yes", "draft", "pull the abstract", "hold", "next step"]) else 0)

        if "thank you for contacting" in body_lower or "team will respond shortly" in body_lower:
            engagement = 1

        return json.dumps({
            "specificity": min(10, specificity),
            "specificity_reason": "Local heuristic: counted concrete anchors and source/offer references.",
            "category_fit": min(10, category_fit),
            "category_fit_reason": "Local heuristic: checked category-specific vocabulary and tone cues.",
            "merchant_fit": min(10, merchant_fit),
            "merchant_fit_reason": "Local heuristic: checked merchant personalization markers.",
            "decision_quality": min(10, decision_quality),
            "decision_quality_reason": "Local heuristic: checked whether the trigger and CTA are reflected in the body.",
            "engagement_compulsion": min(10, engagement),
            "engagement_reason": "Local heuristic: checked for low-friction asks, reciprocity, and direct next steps.",
            "hint": "Use one concrete anchor, one merchant-specific hook, and one clear next action."
        })
...
class DatasetLoader:
    def __init__(self, dataset_dir: Path):
        ...
        self.dataset_dir = expanded_dir if expanded_dir.exists() else dataset_dir
        self.categories = {}
        self.merchants = {}
        self.customers = {}
        self.triggers = {}
        self.test_pairs: List[Dict[str, Any]] = []
    ...
    def load(self) -> bool:
        try:
            ...
            expanded_layout = (self.dataset_dir / "merchants").exists() and (self.dataset_dir / "customers").exists() and (self.dataset_dir / "triggers").exists()

            if expanded_layout:
                for container, key in [("merchants", "merchant_id"), ("customers", "customer_id"), ("triggers", "id")]:
                    storage = getattr(self, container)
                    for path in sorted((self.dataset_dir / container).glob("*.json")):
                        with open(path, encoding="utf-8") as fp:
                            item = json.load(fp)
                        if key in item:
                            storage[item[key]] = item

                test_pairs_path = self.dataset_dir / "test_pairs.json"
                if test_pairs_path.exists():
                    with open(test_pairs_path, encoding="utf-8") as fp:
                        pairs_data = json.load(fp)
                    self.test_pairs = pairs_data.get("pairs", [])
            else:
...
class JudgeSimulator:
    ...
    def run(self, scenario: str) -> bool:
        ...
        data, err, _ = self.client.teardown()
        if not err and data and data.get("accepted"):
            print_info("Reset bot state via /v1/teardown before warmup")
